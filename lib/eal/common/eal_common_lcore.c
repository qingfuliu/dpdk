...
int rte_eal_cpu_init(void)
{
	...
	struct rte_config *config = rte_eal_get_configuration(); // 全局配置 rte_config
	unsigned lcore_id;
	unsigned count = 0;
	...
	for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++)
	{
		...
		{
			unsigned int _i;
			for (_i = 0; _i < (((128) + ((sizeof(long long) * 8) - 1)) / (sizeof(long long) * 8)); _i++)
				(&lcore_config[lcore_id].cpuset)->_bits[_i] = 0LL;
		} while (0);
