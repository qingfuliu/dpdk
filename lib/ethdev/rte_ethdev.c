...
rte_eth_iterator_next(struct rte_dev_iterator *iter)
{
	...
	do
	{ /* loop to try all matching rte_device */
...
rte_eth_dev_count_avail(void)
{
	...
	for (p = rte_eth_find_next_owned_by(0, 0); (unsigned int)p < (unsigned int)32; p = rte_eth_find_next_owned_by(p + 1, 0))
		count++;
						  ...
						  const struct rte_eth_conf *dev_conf)
{
	...
	if (dev_conf->rxmode.mtu == 0)
		dev->data->dev_conf.rxmode.mtu =
			(dev_info.max_mtu == 0) ? RTE_ETHER_MTU : RTE_MIN(dev_info.max_mtu, RTE_ETHER_MTU);
		...
		dev_conf->rxmode.offloads)
	{
		...
		RTE_ETHDEV_LOG_LINE(ERR, "Ethdev port_id=%u does not support Rx offloads %s",
							port_id, eth_dev_offload_names(dev_conf->rxmode.offloads & ~dev_info.rx_offload_capa, buffer, sizeof(buffer), rte_eth_dev_rx_offload_name));
		RTE_ETHDEV_LOG_LINE(DEBUG, "Ethdev port_id=%u was requested Rx offloads %s",
							port_id, eth_dev_offload_names(dev_conf->rxmode.offloads, buffer, sizeof(buffer), rte_eth_dev_rx_offload_name));
		RTE_ETHDEV_LOG_LINE(DEBUG, "Ethdev port_id=%u supports Rx offloads %s",
							port_id, eth_dev_offload_names(dev_info.rx_offload_capa, buffer, sizeof(buffer), rte_eth_dev_rx_offload_name));
		...
		dev_conf->txmode.offloads)
	{
		...
		RTE_ETHDEV_LOG_LINE(ERR, "Ethdev port_id=%u does not support Tx offloads %s",
							port_id, eth_dev_offload_names(dev_conf->txmode.offloads & ~dev_info.tx_offload_capa, buffer, sizeof(buffer), rte_eth_dev_tx_offload_name));
		RTE_ETHDEV_LOG_LINE(DEBUG, "Ethdev port_id=%u was requested Tx offloads %s",
							port_id, eth_dev_offload_names(dev_conf->txmode.offloads, buffer, sizeof(buffer), rte_eth_dev_tx_offload_name));
		RTE_ETHDEV_LOG_LINE(DEBUG, "Ethdev port_id=%u supports Tx offloads %s",
							port_id, eth_dev_offload_names(dev_info.tx_offload_capa, buffer, sizeof(buffer), rte_eth_dev_tx_offload_name));
		ret = -EINVAL;
		goto rollback;
	}
						   ...
						   struct rte_mempool *mp)
{
	...
	if ((mp != NULL) +
			...
			(rx_conf != NULL && rx_conf->rx_nmempool > 0) !=
		1)
	{
		RTE_ETHDEV_LOG_LINE(ERR,
...
int rte_eth_link_to_str(char *str, size_t len, const struct rte_eth_link *eth_link)
{
	int ret;
...
int rte_eth_dev_set_tx_queue_stats_mapping(uint16_t port_id, uint16_t tx_queue_id,
										   uint8_t stat_idx)
{
	int ret;
...
int rte_eth_dev_set_rx_queue_stats_mapping(uint16_t port_id, uint16_t rx_queue_id,
										   uint8_t stat_idx)
{
	int ret;
...
int rte_eth_dev_info_get(uint16_t port_id, struct rte_eth_dev_info *dev_info)
{
	...
	do
	{
		if (!rte_eth_dev_is_valid_port(port_id))
		{
			do
			{
				rte_log(4U, rte_eth_dev_logtype, "ETHDEV"
												 ": "
												 ""
												 "Invalid port_id=%u"
												 "\n"
...
int rte_eth_find_rss_algo(const char *name, uint32_t *algo)
{
	unsigned int i;
...
int rte_eth_dev_default_mac_addr_set(uint16_t port_id, struct rte_ether_addr *addr)
{
	struct rte_eth_dev *dev;
	int index;
	int ret;
...
int rte_eth_recycle_rx_queue_info_get(uint16_t port_id, uint16_t queue_id,
...
int rte_eth_timesync_read_rx_timestamp(uint16_t port_id, struct timespec *timestamp,
									   uint32_t flags)
{
	struct rte_eth_dev *dev;
	int ret;
										 ...
										 struct rte_eth_ip_reassembly_params *reassembly_capa)
{
	...
	ret = eth_err(port_id, (*dev->dev_ops->ip_reassembly_capability_get)(dev, reassembly_capa));
